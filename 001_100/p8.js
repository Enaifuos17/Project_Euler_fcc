/*
  ...
  * P8 - Largest product in a series
    ...
    Find the n adjacent digits in the 1000-digit number 
    that have the greatest product. 

    What is the value of this product?

    - steps to solve the problem
    -- get the series of digits
    -- get n (how many digits to get their product)
    -- iterate through the series
    -- create while looping an array contain
        the current item of the array TO the n
    -- Check if the array's length equal to n
        BCS we need to get the product of n digits
    -- Iterate through the array
    -- Calculate the product of the n digits
    -- BTW we need to create a variable to set in it
        the largest product
    -- if the product of the n digits GREATER THAN variable
...
function largestProductinaSeries(n) {
  ...
  // Iterate through the thousandDigits
  for (let i = 0; i < thousandDigits.length; i++) {
    // Initialize with every loop
    let currentProdut = 1;

    // get the first n digits TO GET their product
    currentDigits = thousandDigits.slice(i, i + n);

    if (currentDigits.length !== n) {
      break; // stop the loop
    }

    // Iterate through the currentDigits
    for (let j = 0; j < currentDigits.length; j++) {
      currentProdut *= currentDigits[j];
    }

    // push into the allProducts array
    allProducts.push(currentProdut);
  }
